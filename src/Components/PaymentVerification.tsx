...
const AdminPaymentVerification = () => {
  ...
  const renderWalletFlow = (payment: Payment) => {
    ...
    // Get the phone number to copy based on sender type
    const phoneNoToCopy =
      payment.sender === 'SELLER' || payment.sender === 'CUSTOMER'
    ...
    const handleCopyPhoneNo = () => {
      navigator.clipboard.writeText(phoneNoToCopy)
      // You might want to add some toast/notification here to indicate success
    }

    return (
      <div className='flex items-center text-xs'>
        <div className='text-[10px] font-medium text-gray-700 flex items-center'>
          {senderWallet}
          <button
            onClick={handleCopyPhoneNo}
